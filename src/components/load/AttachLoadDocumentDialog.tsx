...
import type { LoadUpdateData } from '@/types';
...
interface AttachLoadDocumentDialogProps {
  isOpen: boolean;
  setIsOpen: (isOpen: boolean) => void;
  loadId: string;
  loadIdentifier: string;
  documentTypeField: keyof LoadUpdateData; // e.g., 'outturnReportDocumentName'
...
export default function AttachLoadDocumentDialog({
  isOpen,
  setIsOpen,
  loadId,
  loadIdentifier,
  ...
  const onSubmit: SubmitHandler<AttachDocumentFormData> = (data) => {
    ...
    if (newFile) {
      onDocumentAction(loadId, documentTypeField, newFile.name);
      toast({
        title: `${documentFriendlyName} Updated!`,
        description: `'${newFile.name}' has been associated with load ${loadIdentifier}.`,
      });
  ...
  const handleClearDocument = () => {
    onDocumentAction(loadId, documentTypeField, null); // Pass null to indicate clearing
    toast({
      title: `${documentFriendlyName} Cleared!`,
      description: `The ${documentFriendlyName.toLowerCase()} has been disassociated from load ${loadIdentifier}.`,
      variant: 'destructive',
    });
    reset();
    setIsOpen(false);
  ...
  const dialogDescription = `Upload or clear the ${documentFriendlyName.toLowerCase()} for load: ${loadIdentifier}.`;
