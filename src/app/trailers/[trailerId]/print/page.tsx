...
import type { Load, Shipment } from '@/types';
...
export default function PrintLoadTransferPage() {
  const router = useRouter();
  const params = useParams();
  const loadId = params.loadId as string;
  const { toast } = useToast();

  const { getLoadById, getShipmentsByLoadId, updateLoad } = useWarehouse();
  ...
  const handleSaveACPForm = () => {
    ...
    const acpDocName = `ACP_Form_${load.id}_${timestamp}.pdf`;
    updateLoad(load.id, { acpDocumentName: acpDocName });
    toast({
      title: "ACP Form Saved",
      description: `Associated '${acpDocName}' with load ${load.id}. You can manage this in 'Edit Load'.`,
    });
  };
  ...
  const manifestRef = load.id;
  const unitContainerNumber = load.name;
  const t1_1 = load.customField1;
  const t1_2 = load.customField2;
  const arrivalDateFormatted = formatDateForForm(load.arrivalDate);
  const totalShipments = shipments.length;
  const clearanceAgencyCompany = load.company;
      ...
      <Card className="printable-area shadow-lg print:shadow-none print:border-none">
        ...
        <CardContent className="pt-3 text-xs print:text-[8pt] print:pt-1 space-y-3 print:space-y-1">
          ...
          {/* Part C */}
           <div className="space-y-1 print:space-y-0.5 border border-foreground print:border-black p-2 print:p-1">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 print:gap-y-0">
                ...
                <FormLine label="Container/Load Number" value={unitContainerNumber} valueBold/>
          ...
          <div className="text-xs print:text-[7pt] mt-2 print:mt-1 space-y-0.5 print:leading-tight">
            ...
            <p>The container/load must be delivered direct to the Temporary Storage Facility above. If seals are affixed, they are not to be broken without Customs authorisation.</p>
